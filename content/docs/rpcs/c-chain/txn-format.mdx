...
[secp256k1](/docs/rpcs/other/standards/cryptographic-primitives#cryptography-in-the-lux-virtual-machine)
...
- **`Amount`** is the amount of the asset to be transferred (specified in nLUX
  for LUX and the smallest denomination for all other assets).
...
- **`networkID`** is an int that defines which Lux network this
  transaction is meant to be issued to. This could refer to Mainnet, Testnet, etc.
