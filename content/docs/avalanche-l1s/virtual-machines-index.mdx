...
To reach a consensus on linear blockchains, Lux uses the Snowman consensus engine. To be compatible with Snowman, a VM must implement the `block.ChainVM` interface.

For more information, see [here](https://github.com/luxfi/luxgo/blob/master/snow/engine/snowman/block/vm.go).
...
The `snowman.Block` interface It define the functionality a block must implement to be a block in a linear Snowman chain. For more information, you can see the full file [here](https://github.com/luxfi/luxgo/blob/master/snow/consensus/snowman/block.go).
