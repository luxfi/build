...
Let's write our VM, which implements `block.ChainVM` and whose blocks implement `snowman.Block`. You can also follow the code in the [TimestampVM repository](https://github.com/luxfi/timestampvm/tree/main).
...
```bash
cp -n <path to your binary> $GOPATH/src/github.com/luxfi/luxgo/build/plugins/
```
