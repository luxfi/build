...
description: This page demonstrates how to deploy Lux validators on AWS using just one Lux-CLI command.
---

This page demonstrates how to deploy Lux validators on AWS using just one Lux-CLI command.
...
Finally, if monitoring is set up, Lux-CLI will also output the Grafana link where the 
...
- Started the process of bootstrapping your new Lux node to the Primary Network (for non-Devnet only).
