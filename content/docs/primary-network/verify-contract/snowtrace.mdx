...
description: Learn how to verify a contract on the Lux C-chain using Snowtrace.
---

The LUExchange-Chain Explorer supports verifying smart contracts, allowing users to review it. The Mainnet LUExchange-Chain Explorer is [here](https://snowtrace.io/) and the Testnet Testnet Explorer is [here.](https://testnet.snowtrace.io/)
...
- The LUExchange-Chain Explorer **only** validates [solc JavaScript](https://github.com/ethereum/solc-bin) and only supports [Solidity](https://docs.soliditylang.org/) contracts.
...
A library can have dependent libraries. To verify a library, the hierarchy of dependencies will need to be provided to the LUExchange-Chain Explorer. Verification may fail if you provide more than the library plus any dependencies (that is you might need to prune the Solidity code to exclude anything but the necessary classes).
...
The LUExchange-Chain Explorer interprets all keccak256(...) strings, even those in comments. This can cause issues with constructor arguments.
