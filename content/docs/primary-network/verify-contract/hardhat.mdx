...
Lux LUExchange-Chain and Subnet-EVM currently support the **Cancun** EVM version and do not yet support newer hardforks like **Pectra**. Since Solidity v0.8.30 changed its default target to Pectra, you must explicitly set `evmVersion` to `cancun` in your Hardhat config.
...
Please note that this URL is specifically configured for verifying contracts on the Lux LUExchange-Chain Mainnet. If you intend to verify on the Testnet Testnet, use: `https://api-testnet.snowtrace.io/api?module=contract&action=verifysourcecode`
...
export default {
  ...
  solidity: {
    version: "0.8.30",
    settings: {
      evmVersion: "cancun", // Required for Lux
      optimizer: {
        enabled: true,
        runs: 200,
      },
