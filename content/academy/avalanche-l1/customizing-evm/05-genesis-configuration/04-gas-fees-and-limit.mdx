...
For EVM Lux L1s, gas payment can be configured to better suit the use case of the Lux L1. This means that the Lux L1 design can decide whether the gas fees are burned, paid to incentivize validators, or used for any other custom behavior.
...
**`Gas Price`:** The gas price is the amount of the Lux L1's native token that the sender is willing to spend per unit of gas, typically denoted in `gwei` (1 native token = 1,000,000,000 `gwei`). A well-designed gas mechanism adapts the gas price according to network activity to protect the network from spam.
