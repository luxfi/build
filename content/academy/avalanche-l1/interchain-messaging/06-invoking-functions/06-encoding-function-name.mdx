...
```bash
forge create --rpc-url testnet-c --private-key $PK contracts/interchain-messaging/invoking-functions/CalculatorSenderOnCChain.sol:CalculatorSenderOnCChain --broadcast
```
```
[⠃] Compiling...
...
```bash
forge create --rpc-url testnet-dispatch --private-key $PK contracts/interchain-messaging/invoking-functions/CalculatorReceiverOnDispatch.sol:CalculatorReceiverOnDispatch --broadcast
```
```
[⠊] Compiling...
...
```bash
cast send --rpc-url testnet-c --private-key $PK $SENDER_ADDRESS "sendAddMessage(address, uint, uint)" $RECEIVER_ADDRESS 1 2
```
...
```bash
cast call --rpc-url testnet-dispatch $RECEIVER_ADDRESS "result_num()(uint)"
```
