...
Next, the AWM Relayer delivers the message with the aggregated signature by simply submitting a transaction to the destination Lux L1. The transaction is propagated through the validator set of the destination blockchain just as any other regular transaction would be.
...
### Query the Platform-Chain:
The verifying validators of the destination L1 each query the validator set of the source Lux L1 with its stake weights and BLS Public Keys.
