...
Cross-Chain dApp initiating the message calls the Interchain Messaging Contract on the source Lux L1
...
### Message Relaying
An AWM Relayer relays the message to the destination Chain. It periodically checks the source Lux L1 for outgoing messages and delivers these by calling the Interchain Messaging contract on the destination Lux L1.
...
The Interchain Messaging contract on the destination chain interacts with AWM to verify the signature of the message and whether it has been signed by a sufficiently large stake share of the source Lux L1's validator set.
