...
In this example, we will wrap Trader Joe's Factory contract to execute swap operations on the destination chain. Trader Joe's exchange contracts are already deployed on Testnet, which why we are choosing Testnet as the destination chain.
...
// (c) 2024, Lux Network, Inc. All rights reserved.
...
import {IWLUX} from "./interface/IWLUX.sol";
...
contract DexERC20Wrapper is Context, IERC20SendAndCallReceiver {
    ...
    function receiveTokens(
        ...
        if (swapOptions.tokenOut == address(0)) {
            ...
            IWLUX(WNATIVE).withdraw(amountOut);
