...
> In the following commands, make sure to substitute the name of your Lux L1 for `<name>` (including the `<>`). You can choose whatever name you want, but I recommend using `myblockchain` as the name to avoid the need for some manual configuration.

<Steps>
<Step>
 
### Start Lux L1 Configuration
To create an Lux L1 configuration run the following command:

```bash
lux blockchain create <name> --sovereign=false
```

For example:

```bash
lux blockchain create myblockchain --sovereign=false
```
...
You will be prompted a wizard to configure your Lux L1. In the first question, select `Subnet-EVM`:
...
```
@martineckardt ➜ /workspaces/lux-starter-kit (main) $ lux blockchain create myblockchain
✔ Subnet-EVM
✔ I want to use defaults for a test environment
Chain ID: 9999
Token Symbol: TOK
...
This command creates the config files `genesis.json` and `sidecar.json` inside: `~/.lux-cli/subnets/<name>`. If you want, you can retrieve the current config with the following command:
...
```
@martineckardt ➜ /workspaces/lux-starter-kit (main) $ lux blockchain deploy myblockchain
✔ Local Network
Deploying [myblockchain] to Local Network
Backend controller started, pid: 5317, output at: /home/vscode/.lux-cli/runs/server_20240808_164902/lux-cli-backend.log
Installing luxgo-v1.11.10...
luxgo-v1.11.10 installation successful

Booting Network. Wait until healthy...
Node logs directory: /home/vscode/.lux-cli/runs/network_20240808_164903/node<i>/logs
...
/*
Commenting this because on configure-ports in the devcontainer file, the visibility of the port is already set to public.

<Step>
 
### Make RPC-Endpoint publicly accessible

The final part of the output shows the wallet connection details. You can use these to connect your wallet like Core or Metasmask to you newly created Lux L1.
