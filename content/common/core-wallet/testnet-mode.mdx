import Gallery from '@/components/content-design/gallery';

Since we do not want to play with real funds, we will switch to the Testnet Testnet, an exact replica of the Lux's Mainnet architecture with the difference that its tokens are valueless.

To switch to Testnet Mode, open the Core browser extension, click on the burger menu, then Advanced, and finally activate Testnet Mode.

<Gallery url1="/common-images/core-wallet/go-to-menu.png" url2="/common-images/core-wallet/go-to-advanced.png" />